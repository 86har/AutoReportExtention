...
let videoPauseBtnClicked = false;
...
// function videoBackgroundPlay(video){
//   if(videoPauseBtnClicked){
//     console.log("pause section1.");
//   }else{
//     console.log("pause section2.");
//   }

//   if(settingDatas["functions"]["background_play"] && videoStatus && !(videoPauseBtnClicked)){
//     video.play();
//     video.addEventListener("pause", () => videoBackgroundPlay(video), {once: true});
//   }
// }
  ...
  if (iframeElement) {
    ...
    if(video){
      ...
      if (videoSrc != videoSrcData) {
        ...
        iframeElement.contentWindow.document.getElementsByClassName("sc-192m11g-2")[0].addEventListener("click", ()=>{
          if(videoPauseBtnClicked) setTimeout(()=>{videoPauseBtnClicked = false}, 100);
          videoPauseBtnClicked = true;
        })

        video.addEventListener("pause", () => {
          if(settingDatas["functions"]["background_play"] && videoStatus
          && !(video.currentTime >= video.duration) && !(videoPauseBtnClicked)){
            videoPauseBtnClicked = false;
            video.play();
          }
        });
